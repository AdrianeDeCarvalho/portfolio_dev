# portfolio_dev
Minha coleção de projetos de desenvolvimento, destacando habilidades em Python e resolução de problemas.

---

# Olá! :wave:  Eu sou Adriane de Carvalho.
Sou uma desenvolvedora apaixonada por **Python** e focada em desenvolvimento back-end. Minha jornada na programação é guiada pela curiosidade, busca por soluções eficientes e aprimoramento contínuo das minhas habilidades.

---

## 🚀 Meus Projetos

Aqui estão alguns dos projetos que desenvolvi, focados em resolver problemas reais e aplicar conceitos modernos de arquitetura de software.

| Projeto | Descrição | Tecnologias | Link |
| :--- | :--- | :--- | :--- |
| **Sistema Bancário Pro** | Aplicação financeira focada em integridade de dados, com transferências e extratos validados por testes unitários. | Python, Django, SQLite, Unit Testing | [Visualizar]()
| **Gestão de Tarefas (Django)** | Sistema Web com controle de prioridades, status dinâmicos e persistência em banco de dados. | Python, Django, SQLite, CSS | [Visualizar](https://github.com/AdrianeDeCarvalho/gestao-tarefas-equipe) |
| **Gerenciamento de RH** | Aplicação Back-end para gestão de equipes com interface rica via terminal. | Python, Rich, SQLite | [Visualizar](https://github.com/AdrianeDeCarvalho/sistema-gerenciamento-equipe) |
| **Evolução de Usuários** | Projeto focado em demonstrar a progressão de lógica e boas práticas em Python. | Python, POO | [Visualizar](https://github.com/AdrianeDeCarvalho/sistema-gerenciamento-usuarios) |
| **Gerenciador de Tarefas v1** | De scripts básicos até a versão final com persistência e UI. | Python, Lógica Pura | [Visualizar](https://github.com/AdrianeDeCarvalho/gerenciador-de-tarefas) |

---

Estou em busca de novos desafios!

---

## 🛠️ Minhas Habilidades

* **Linguagens & Frameworks:** Python (Django, Flask), Noções de Java.
* **Back-end & Arquitetura:** Padrão MVT (Model-View-Template), Desenvolvimento de APIs, Autenticação e Autorização.
* **Banco de Dados:** SQLite, Modelagem de Dados, Django ORM (Queries e Relacionamentos).
* **Conceitos & Paradigmas:** Orientação a Objetos (POO), Lógica de Programação, Estrutura de Dados, Tratamento de Exceções.
* **Ferramentas & DevOps:** Git, GitHub, Ambientes Virtuais (venv), Gerenciamento de Dependências (pip).

---

## 🗺️ Roadmap de Evolução Técnica (Próximos 2 Meses)

"Para elevar o nível das minhas soluções, tracei um plano de estudos focado em transformar aplicações funcionais em sistemas robustos, seguros e escaláveis."

| Fase | Foco Tecnológico | Objetivo |
| :--- | :--- | :--- |
| **01. Consolidação** | Relacionamentos (ForeignKey) | Implementar relações complexas entre modelos e otimização de queries com o ORM. |
| **02. Arquitetura API** | Django REST Framework (DRF) | Transição para o desenvolvimento de APIs RESTful, separando Back-end e Front-end. |
| **03. Segurança** | Autenticação JWT | Implementar camadas de segurança modernas com JSON Web Tokens para controle de acesso. |
| **04. Robustez** | Pydantic & Testes | Garantir integridade de dados com validações avançadas e implementação de Testes Unitários. |

---

---

## 📫 Como me Conectar
* **LinkedIn:** [LinkedIn/adriane-de-carvalho](https://www.linkedin.com/in/adriane-de-carvalho12)
* **Email:** [adrianedecarvalho578@gmail.com](mailto:adrianedecarvalho578@gmail.com)
* **WhatsApp:** [Fale Comigo no WhatsApp](https://wa.me/5551995810356?text=Olá%20Adriane,%20vi%20seu%20portfólio%20no%20GitHub!)

---

Feito com ❤️ por Adriane de Carvalho!
